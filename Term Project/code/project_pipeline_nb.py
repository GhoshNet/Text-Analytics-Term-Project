...
def main():
    ...
    # Save results
    summary_df.to_csv('results_cv_nb.csv', index=False)
    fold_df.to_csv('results_cv_nb_folds.csv', index=False)
    ...
    pairwise_df.to_csv('statistical_tests_nb.csv', index=False)

    print("\nResults saved to:")
    print("  results_cv_nb.csv          — mean/std summary per scenario")
    print("  results_cv_nb_folds.csv    — per-fold raw values")
    print("  statistical_tests_nb.csv   — Friedman + Wilcoxon/Holm pairwise tests")
